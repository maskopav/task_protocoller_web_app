// frontend/src/components/AdminManagement/UserSiteTable.jsx
import React from "react";
import { useTranslation } from "react-i18next";
import DeleteIcon from "../Icons/DeleteIcon";
import "./AdminManagement.css";

export default function UserSiteTable({ assignments, onRemove }) {
  const { t } = useTranslation(["admin", "common"]);

  return (
    <section className="section card">
      <div className="section-header-row">
        <h3 className="section-title">{t("management.siteAssignments.title")}</h3>
      </div>
        ...
        <table className="table">
          <thead>
            <tr>
              ...
              <th>{t("management.siteAssignments.table.site")}</th>
          ...
          <tbody>
            {assignments.map((a) => (
              <tr key={a.assignment_id}>
                ...
                <td>{a.user_email}</td>
                <td><span className="project-tag">{a.site_name}</span></td>
            ...
            {assignments.length === 0 && (
              <tr><td colSpan="5" className="empty-row">{t("management.siteAssignments.noData")}</td></tr>
            )}
          </tbody>
        </table>
      </div>
