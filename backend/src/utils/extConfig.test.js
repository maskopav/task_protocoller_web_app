...
describe('buildExtConfig — case specifics', () => {
  ...
  it('multi_project: flat protocols, each naming its parent project', () => {
    const { config } = all.multi_project;
    expect(config).not.toHaveProperty('projects');
    expect(config.protocols.map((p) => [p.project, p.name])).toEqual([
  ...
  it('skipped_vision: serves the protocol without the unsupported task', () => {
    ...
    expect(config.protocols[0].tasks).toHaveLength(1);
  });
  ...
  it('serves a site with no protocols as an empty but valid config', () => {
    ...
    expect(config).toMatchObject({ languages: ['de'], defaultLanguage: 'de', protocols: [], strings: { de: {} }, configVersion: '2026-01-01.000000' });
  });
  ...
  it('omits audioExamplePath when ASSET_BASE_URL is not configured', () => {
    ...
    for (const t of config.protocols[0].tasks) expect(t.audioExamplePath).toBeUndefined();
  });
});
